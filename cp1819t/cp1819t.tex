...
\\
\>[B]{}\Varid{find}\mathrel{=}\Varid{curry}\;(\Varid{auxfind}\comp (\Varid{id}\times\Varid{tar})){}\<[E]%
\\[\blanklineskip]%
\>[B]{}\Varid{auxfind}\mathbin{::}(\Conid{Eq}\;\Varid{a})\Rightarrow (\Varid{a},[\mskip1.5mu (\Conid{Path}\;\Varid{a},\Varid{b})\mskip1.5mu])\to [\mskip1.5mu \Conid{Path}\;\Varid{a}\mskip1.5mu]{}\<[E]%
\\
\>[B]{}\Varid{auxfind}\;(\Varid{a},[\mskip1.5mu \mskip1.5mu])\mathrel{=}[\mskip1.5mu \mskip1.5mu]{}\<[E]%
\\
\>[B]{}\Varid{auxfind}\;(\Varid{a},(\Varid{l},\anonymous )\mathbin{:}\Varid{xs})\mathrel{=}\mathbf{if}\;(\Varid{last}\;(\Varid{l})\equiv \Varid{a})\;\mathbf{then}\;\Varid{l}\mathbin{:}\Varid{auxfind}\;(\Varid{a},\Varid{xs})\;\mathbf{else}\;\Varid{auxfind}\;(\Varid{a},\Varid{xs}){}\<[E]%
\ColumnHook
\end{hscode}\resethooks
...
Definimos um anamorfismo sobre Exp que a cada elemento presente no sistema de ficheiros, $\ensuremath{(\Varid{a},\Conid{Node}\;\Varid{a}\;\Varid{b})}$ através da função inTerm obtemos pares nome e sistema de ficheiros inferior, que será vazio caso seja um ficheiro.
